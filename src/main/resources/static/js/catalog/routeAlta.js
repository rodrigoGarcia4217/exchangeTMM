...
const generarCost = (routeId) => new Promise((resolve, reject) => {
    ...
    $.ajax({
        type: "POST",
        url: "routeAlta/calculate", // Cambia esto a la URL correcta de tu API
        data: JSON.stringify({
            routeId: routeId,
        }),
...
const fillOriginCreate = () => {
    if($("#createLocalityOrigin").val() != 0) {
        $.ajax({
            type: "GET",
            url: "routeAlta/getSingleDestiny",
            data: {
