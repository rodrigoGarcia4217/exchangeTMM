...
const configDataTable = () => {
    $("#routeTable").DataTable({
        ...
        columns: [
            ...
            { data: 'routeTrailerType', visible: true },
            { data: 'routeType', visible: true, render: function(data, type, row) {
                    if (data === 'SEN') {
                        return 'Sencillo';
                    } else if (data === 'MULTI') {
                        return 'Multidestino';
                    }
                    return data; // Devuelve el valor original si no coincide
                }
            },
            ...
            { 
                data: 'routeId', 
                render: (data) => `<button onclick="generarCost('${data}')"> Actualizar Costos </button>` 
            },
        ]
    });
...
const generarCost = (routeId) => new Promise((resolve, reject) => {
    ...
    $.ajax({
        type: "POST",
        url: "routeLoza/calculate", // Cambia esto a la URL correcta de tu API
        data: JSON.stringify({
            routeId: routeId,
        }),
...
const initComponents = () => {
	$("#editRouteForm").submit( () => {
        ...
        formData.append('routeTrailerType', $("#editTrailerType").val());
        formData.append('routeType', $("#editServiceType").val());
	...
	$("#createRouteForm").submit( () => {
        ...
        formData.append('routeTrailerType', $("#createTrailerType").val());
        formData.append('routeType', $("#createServiceType").val());
...
const openEditPopup = (data) => {
    ...
    $.ajax({
        ...
        success: (response) => {
            ...
            $("#editTrailerType").val(response.routeTrailerType);
            $("#editServiceType").val(response.routeType);
...
const fillOriginCreate = () => {
    if($("#createLocalityOrigin").val() != 0) {
        $.ajax({
            type: "GET",
            url: "routeLoza/getSingleDestiny",
            data: {
